...
<span style="display:none!important;visibility:hidden;mso-hide:all;font-size:1px;color:#fff;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">{{ Str::limit(strip_tags($post->content), 80) }} — New post from {{ $post->ownerOfPost->name }}</span>
...
<div class="email-container mx-auto my-8 bg-white shadow-md rounded-xl overflow-hidden">
        ...
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-full bg-white/20 flex items-center justify-center overflow-hidden">
                @if(isset($post->ownerOfPost->avatar))
                    <img src="/storage/images/avatars/{{ $post->ownerOfPost->avatar }}" alt="{{ $post->ownerOfPost->name }}" class="w-14 h-14 object-cover rounded-full">
                @else
    ...
    <div class="p-6">
        ...
        <p class="text-gray-700 mb-4">{{ $post->ownerOfPost->name }} just published a new post. Here’s a quick preview:</p>
        ...
        <div class="mb-6">
            <a href="{{route('posts.show', ['post' => $post->id])}}}" class="btn bg-indigo-600 text-white" style="background-color:#4f46e5;color:#fff;border-radius:8px;padding:12px 20px;">Read the full post</a>
        </div>

        <p class="text-sm text-gray-600">Want to hear more from <strong>{{ $post->ownerOfPost->name }}</strong>? Keep following them to get the latest updates.</p>
