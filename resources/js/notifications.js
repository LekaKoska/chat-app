if (window.auth?.user?.id && window.Echo) {
    ...
    function prependMessageItem(n) {
        ...
        a.href = n.url || "#";
        a.className =
            "block px-4 py-2 flex items-center space-x-3 bg-white dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800 transition";

        const avatarPath = n.sender?.avatar
            ? `/storage/images/avatars/${n.sender.avatar}`
            : "/storage/images/avatars/default.png";
